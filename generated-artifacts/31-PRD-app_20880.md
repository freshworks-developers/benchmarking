# PRD-31: Freshsales Backend app for all pods in IPaaS (20880)

**Version:** 1.0  
**Status:** Draft for benchmark review  
**Source package:** `app_20880_v711906.zip`
...
- Core features: Backend app which listens to freshsales's product events. │   ├── iparams.json           Installation parameter config.
...
Build a Freshworks Platform 3.0 app for freshsales / freshworks_crm based on app 20880 (Freshsales Backend app for all pods in IPaaS). Implement: Backend app which listens to freshsales's product events. │   ├── iparams.json           Installation parameter config.. Keep folder structure compatible with: config, server. Include robust error handling, idempotent side effects for event flows, and validation-safe configuration.
...
- Platform features observed: freshsales:events(onAppUninstall, onContactCreate, onContactDelete, onContactUpdate, onLeadCreate, onLeadDelete, onLeadUpdate, onSalesAccountCreate, onSalesAccountDelete, onSalesAccountUpdate); freshworks_crm:events(onAppUninstall, onContactCreate, onContactDelete, onContactUpdate, onSalesAccountCreate, onSalesAccountDelete, onSalesAccountUpdate)
