# PRD-30: Freshdesk Backend app for all pods in IPaaS (20878)

**Version:** 1.0  
**Status:** Draft for benchmark review  
**Source package:** `app_20878_v711942.zip`
...
- Build for: **freshdesk**
- Core features: Backend app which listens to freshdesk's product events. │   ├── iparams.json           Installation parameter config.
...
Build a Freshworks Platform 3.0 app for freshdesk based on app 20878 (Freshdesk Backend app for all pods in IPaaS). Implement: Backend app which listens to freshdesk's product events. │   ├── iparams.json           Installation parameter config.. Keep folder structure compatible with: config, server. Include robust error handling, idempotent side effects for event flows, and validation-safe configuration.
...
- Platform features observed: freshdesk:events(onAppUninstall, onCompanyCreate, onCompanyDelete, onCompanyUpdate, onContactCreate, onContactUpdate)
