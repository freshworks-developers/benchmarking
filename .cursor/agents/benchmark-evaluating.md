...
**Data and instructions:** You work against an **app_id** (e.g. APP001) or **app path** (e.g. `test-apps/MyApp`). If neither is provided, **ask**: *"Please provide the app id (e.g. APP001) or the path to the app directory (e.g. test-apps/MyApp) to evaluate."* Invoke with app_id or path from the user or from prior chat context.
...
5. **Do not change app code by default.** You run evaluation and report. Only modify the app if the user explicitly asks you to fix issues and re-evaluate; otherwise give the developer a concise failure summary and point to the result JSON.
...
1. **Do not change app code unless asked.** You run evaluation and interpret results. For fixes, describe what failed and let the developer change the app (or they may ask you to fix and re-eval).
