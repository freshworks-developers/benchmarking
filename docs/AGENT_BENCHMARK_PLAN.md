...
This repo is the **Freshworks Platform 3.0 evaluation framework**. Agents and humans use it to validate and score Freshworks marketplace apps (FDK validation, Platform 3.0 compliance, Crayons UI, file structure, **Expected Platform Features** from criteria) and to improve over time via error learning and evaluation history.
...
- **Output**:
  - **Result file**: `results/<app_id>_result.json` (schema: `score` including `components`, `validation`, `platform3_compliance`, `file_structure`, `crayons_usage`, `expected_platform_features_check`).
